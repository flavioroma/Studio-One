export type Language = 'en' | 'it';

export const translations = {
    en: {
        common: {
            backToHome: 'Back to Home',
            activeTool: 'Active Tool',
            moreTools: 'More tools coming...',
            openTool: 'Open Tool',
            eraseProject: 'Erase the project',
            eraseConfirm: 'Are you sure you want to erase the project? This action cannot be undone.',
            cancel: 'Cancel',
            removeFile: 'Remove file',
            restartPreview: 'Restart Preview',
            yesRemove: 'Yes, Remove',
            resolution: 'Resolution',
            mediaCreated: 'Creation Date',
            addMore: 'Add More',
        },
        captions: {
            captionText: 'Caption Text',
            aiCaption: 'AI Caption',
            thinking: 'Thinking...',
            enterOverlayText: 'Enter overlay text...',
            textStyle: 'Text Style',
            normal: 'Normal',
            italic: 'Italic',
            textSize: 'Text Size',
            position: 'Position',
            colorPalette: 'Color Palette',
            textPositions: {
                'Top Center': 'Top Center',
                'Top Left': 'Top Left',
                'Top Right': 'Top Right',
                'Bottom Center': 'Bottom Center',
                'Bottom Left': 'Bottom Left',
                'Bottom Right': 'Bottom Right',
                'Center': 'Center',
            },
            textSizes: {
                'Small': 'Small',
                'Large': 'Large',
            },
        },
        watermark: {
            title: 'Watermark',
            upload: 'Upload Watermark',
            remove: 'Remove',
            position: 'Position',
            size: 'Size',
            opacity: 'Opacity',
        },
        home: {
            title: 'Studio One',
            subtitle: 'A collection of tools designed to enhance multimedia content',
            badges: {
                updated: 'Updated',
                popular: 'Popular',
                new: 'New',
            },
            audiotrim: {
                description: 'Trim audio with millisecond precision. Export audio segments while maintaining original quality. Can extract audio from a video file.',
            },
            slidesync: {
                description: 'Create music videos from audio and images. Images can be adjusted, captions and watermarks can be added.',
            },
            videoverlay: {
                description: 'Rotate, replace audio, add overlay text captions and watermarks onto a video.',
            },
            photoverlay: {
                description: 'Overlay text captions and watermarks onto multiple photos.',
            },
            credits: 'Created by Flavio Biundo - Powered by Google Antigravity',
            clientSideTitle: '100% Client-Side',
            clientSideDesc: 'No servers involved - All processing happens directly in your browser, keeping your data private',
        },
        tools: {
            audiotrim: {
                title: 'AudioTrim',
                decodeError: 'Failed to decode audio. Please ensure it\'s a valid audio (MP3, WAV) or video file.',
                dropZoneTitle: 'Select or drop an audio or video file',
                setStart: 'Set Start',
                setEnd: 'Set End',
                selectionStart: 'Selection Start',
                selectionEnd: 'Selection End',
                tip: 'Tip: Use Left/Right keys for 0.1s, Shift for 1s, Alt for 0.01s',
                fullTrack: 'Play Full Track',
                selection: 'Play Selection',
                playFull: 'Play Entire Track',
                playSelection: 'Play Selection Only',
                outputFormat: 'Output Format',
                desc: 'Description',
                smartSlice: 'Smart Bitstream Slice',
                losslessMaster: 'Lossless PCM Master',
                exportAs: 'Export as',
                exportFailed: 'Failed to export audio. Try again.',
                wavDesc: 'Standard lossless PCM format.\nRecommended for absolute millisecond-perfect precision.',
                mp3Desc: 'Compressed audio format.\nNote: For variable bit rate files, selection points might differ slightly, use WAV for absolute accuracy.',
                removeTrackTitle: 'Remove Audio Track?',
                removeTrackMsg: 'Are you sure you want to remove this audio track? This action cannot be undone.',
            },
            slidesync: {
                title: 'SlideSync',
                captionError: 'Could not generate caption. Please check your API key.',
                mediaSettings: 'Media Selection',
                uploadPhotos: '3. Upload Photos',
                addMedia: 'Add Media',
                backgroundMusic: '1. Background Music',
                backgroundMusicDesc: '(sets the duration of the video)',
                selectAudio: 'Select Audio',
                removeAudio: 'Remove Audio',
                videoFormat: '2. Video Format',
                noSlideSelected: 'Select a thumbnail from the timeline to edit its properties',
                timelineSequence: 'Timeline Sequence',
                slidesCount: 'Slides',
                timelineTip: 'Scroll with mouse wheel • Arrows to switch',
                slideProperties: 'Slide Properties',
                framingPreview: 'Framing Preview',
                output: 'Output:',
                panImage: 'Pan Image',
                magnification: 'Magnification',
                centerX: 'Center X',
                centerY: 'Center Y',
                resetFraming: 'Reset Framing',
                noSlidesAdded: 'No slides added yet. Drag and drop images to start.',
                hasText: 'Has text',
                addImagesToStart: 'Add images to start',
                addAudioToStart: 'Add audio to start',
                creating: 'Creating...',
                exportClip: 'Export Clip',
                recordingNote: 'Recording Render... Do not switch tabs.',
                awaitingMedia: 'Awaiting images and audio',
                exportFailed: 'MP4 Export failed. Please check console for details.',
                aiCaptionTitle: 'Generate caption using AI',
                exportAsWebm: 'Export as WebM (VP9)',
                exportAsMp4: 'Export as MP4 (H.264)',
                removeAllDataTitle: 'Remove All Data?',
                removeAllDataMsg: 'Are you sure you want to remove all data? This action cannot be undone.',
                yesRemoveAll: 'Yes, Remove All',
            },
            videoverlay: {
                title: 'VideOverlay',
                overlaySettings: 'Overlay',
                uploadVideo: 'Select or drop a video',
                awaitingSource: 'Awaiting Source',
                removeVideo: 'Remove Video',
                calculating: 'Calculating...',
                highQualityRender: 'High Quality Render',
                processing: 'Processing',
                frameByFrame: 'Frame-by-Frame',
                browserNativeRender: 'Browser-Native Render',
                videoOutputTitle: 'High Quality Video Output',
                videoOutputDesc: 'The output video is generated as MP4, with the same quality as the original video',
                exportVideo: 'EXPORT VIDEO',
                processingProgress: 'Processing',
                generatingHQ: 'Generating High Quality Video',
                hqNote: 'Processing frame-by-frame for maximum quality. This may take longer than the original duration.',
                keepTabActive: 'Please keep this tab active. Switching tabs may pause the rendering process.',
                cancelGeneration: 'Cancel Generation',
                cancelExport: 'Cancel Export',
                cancelConfirm: 'Are you sure you want to cancel the export? All progress will be lost.',
                yesCancel: 'Yes, Cancel',
                noContinue: 'No, Continue',
                abortGenerationTitle: 'Abort Generation?',
                abortGenerationMsg: 'Are you sure you want to cancel the video generation? Progress will be lost.',
                yesAbort: 'Yes, Abort',
                continue: 'Continue',
                removeVideoTitle: 'Remove Video?',
                removeVideoMsg: 'Are you sure you want to remove this video? This action cannot be undone.',
                exportFailed: 'Export failed. See console for details.',
                originalFormat: 'Original',
                videoFormat: 'Video Format',
                rotation: 'Rotation',
                audioSettings: 'Audio',
                keepAudio: 'Keep Audio',
                removeAudio: 'Remove',
                replaceAudio: 'Replace',
                selectAudio: 'Select Audio',
                videoError: 'This video format or codec is not supported by your browser. Older AVI files (~2008 and earlier) often use legacy codecs. Try converting it to MP4 (H.264).',
            },
            photoverlay: {
                title: 'PhotOverlay',
                overlaySettings: 'Overlay',
                selectPhoto: 'Select a photo to add overlays',
                uploadPhotos: 'Select or drop Photos',
                awaitingSource: 'Awaiting Source',
                removePhoto: 'Remove Photo',
                location: 'Location',
                exportPhoto: 'EXPORT PHOTO',
                exporting: 'Exporting...',
                removePhotoTitle: 'Remove Photo?',
                removePhotoMsg: 'Are you sure you want to remove this photo? This action cannot be undone.',
                exportFailed: 'Export failed. See console.',
                panImage: 'Pan Image',
                applyToAll: 'Apply this overlay to all photos',
                exportAllPhotos: 'EXPORT ALL PHOTOS',
                applyToAllTitle: 'Apply to All Photos?',
                applyToAllMsg: 'This will overwrite the custom overlay settings of your other photos. Are you sure?',
                yesApply: 'Yes, Apply',
                removeAllPhotosTitle: 'Remove All Photos?',
                removeAllPhotosMsg: 'Are you sure you want to remove all photos? This action cannot be undone.',
                yesRemoveAll: 'Yes, Remove All',
                exportMode: 'Export Mode',
                individualFiles: 'Individual Files',
                singleArchive: 'Single Archive (ZIP)',
            },
        },
    },
    it: {
        common: {
            backToHome: 'Torna alla Home',
            activeTool: 'Strumento Attivo',
            moreTools: 'Altri strumenti in arrivo...',
            openTool: 'Apri Strumento',
            eraseProject: 'Butta via il progetto',
            eraseConfirm: 'Sei sicuro di voler buttare via il progetto? L\'azione non può essere annullata.',
            cancel: 'Annulla',
            removeFile: 'Rimuovi file',
            restartPreview: 'Riavvia Anteprima',
            yesRemove: 'Sì, Rimuovi',
            resolution: 'Risoluzione',
            addMore: 'Aggiungi Altre',
            mediaCreated: 'Data di creazione',
        },
        captions: {
            captionText: 'Testo Didascalia',
            aiCaption: 'Didascalia AI',
            thinking: 'Elaborazione...',
            enterOverlayText: 'Inserisci testo...',
            textStyle: 'Stile Testo',
            normal: 'Normale',
            italic: 'Corsivo',
            textSize: 'Dimensione',
            position: 'Posizione',
            colorPalette: 'Tavolozza Colori',
            textPositions: {
                'Top Center': 'In alto al centro',
                'Top Left': 'In alto a sinistra',
                'Top Right': 'In alto a destra',
                'Bottom Center': 'In basso al centro',
                'Bottom Left': 'In basso a sinistra',
                'Bottom Right': 'In basso a destra',
                'Center': 'Al centro',
            },
            textSizes: {
                'Small': 'Piccolo',
                'Large': 'Grande',
            },
        },
        watermark: {
            title: 'Filigrana',
            upload: 'Carica Filigrana',
            remove: 'Rimuovi',
            position: 'Posizione',
            size: 'Dimensione',
            opacity: 'Opacità',
        },
        home: {
            title: 'Studio One',
            subtitle: 'Una collezione di strumenti progettati per migliorare contenuti multimediali',
            badges: {
                updated: 'Aggiornato',
                popular: 'Popolare',
                new: 'Nuovo',
            },
            audiotrim: {
                description: 'Taglia audio con precisione al millisecondo. Esporta segmenti audio mantenendo la qualità originale. Può estrarre l\'audio da un file video.',
            },
            slidesync: {
                description: 'Crea video musicali da audio e immagini. Le immagini possono essere regolate e si possono aggiungere didascalie e filigrane.',
            },
            videoverlay: {
                description: 'Ruota, sostituisci audio, aggiungi didascalie di testo e filigrane su un video.',
            },
            photoverlay: {
                description: 'Sovrapponi didascalie di testo e filigrane su molteplici foto.',
            },
            credits: 'Creato da Flavio Biundo - Potenziato da Google Antigravity',
            clientSideTitle: '100% Lato Client',
            clientSideDesc: 'Nessun server coinvolto - L\'elaborazione avviene localmente nel browser, mantenendo i tuoi dati privati',
        },
        tools: {
            audiotrim: {
                title: 'AudioTrim',
                decodeError: 'Impossibile decodificare l\'audio. Assicurati che sia un file audio (MP3, WAV) o video valido.',
                dropZoneTitle: 'Seleziona o trascina un file audio o video',
                setStart: 'Imposta Inizio',
                setEnd: 'Imposta Fine',
                selectionStart: 'Inizio Selezione',
                selectionEnd: 'Fine Selezione',
                tip: 'Suggerimento: Usa le frecce Destra/Sinistra per 0.1s, Shift per 1s, Alt per 0.01s',
                fullTrack: 'Riproduci Traccia Intera',
                selection: 'Riproduci Selezione',
                playFull: 'Riproduci Traccia Intera',
                playSelection: 'Riproduci Solo Selezione',
                outputFormat: 'Formato di Output',
                desc: 'Descrizione',
                smartSlice: 'Taglio Intelligente Bitstream',
                losslessMaster: 'Master PCM Lossless',
                exportAs: 'Esporta',
                exportFailed: 'Esportazione audio fallita. Riprova.',
                wavDesc: 'Formato PCM lossless standard.\nConsigliato per un\'accuratezza millimetrica assoluta.',
                mp3Desc: 'Formato audio compresso.\nNota: per i file con bitrate variabile, i punti di selezione potrebbero differire leggermente, usa WAV per la massima precisione.',
                removeTrackTitle: 'Rimuovere Traccia Audio?',
                removeTrackMsg: 'Sei sicuro di voler rimuovere questa traccia audio? L\'azione non può essere annullata.',
            },
            slidesync: {
                title: 'SlideSync',
                captionError: 'Impossibile generare la didascalia. Controlla la tua chiave API.',
                mediaSettings: 'Seleziona Media',
                uploadPhotos: '3. Carica Foto',
                addMedia: 'Aggiungi Media',
                backgroundMusic: '1. Musica di Sottofondo',
                backgroundMusicDesc: '(stabilisce la durata del video)',
                selectAudio: 'Seleziona Audio',
                removeAudio: 'Rimuovi Audio',
                videoFormat: '2. Formato Video',
                noSlideSelected: 'Seleziona una miniatura dalla timeline per modificarne le proprietà',
                timelineSequence: 'Sequenza Timeline',
                slidesCount: 'Slide',
                timelineTip: 'Scorri con la rotellina del mouse • Frecce per cambiare',
                slideProperties: 'Proprietà Slide',
                framingPreview: 'Anteprima Inquadratura',
                output: 'Output:',
                panImage: 'Sposta Immagine',
                magnification: 'Ingrandimento',
                centerX: 'Centra X',
                centerY: 'Centra Y',
                resetFraming: 'Ripristina Inquadratura',
                noSlidesAdded: 'Nessuna slide aggiunta. Trascina le immagini per iniziare.',
                hasText: 'Contiene testo',
                addImagesToStart: 'Aggiungi immagini per iniziare',
                addAudioToStart: 'Aggiungi audio per iniziare',
                creating: 'Creazione in corso...',
                exportClip: 'Esporta Clip',
                recordingNote: 'Rendering in corso... Non cambiare scheda.',
                awaitingMedia: 'In attesa di immagini e audio',
                exportFailed: 'Esportazione MP4 fallita. Controlla la console per i dettagli.',
                aiCaptionTitle: 'Genera didascalia con AI',
                exportAsWebm: 'Esporta come WebM (VP9)',
                exportAsMp4: 'Esporta come MP4 (H.264)',
                removeAllDataTitle: 'Rimuovi tutto?',
                removeAllDataMsg: 'Sei sicuro di voler rimuovere tutto? L\'azione non può essere annullata.',
                yesRemoveAll: 'Sì, Rimuovi Tutto',
            },
            videoverlay: {
                title: 'VideOverlay',
                overlaySettings: 'Sovraimpressione',
                uploadVideo: 'Seleziona o trascina un video',
                awaitingSource: 'In attesa del video',
                removeVideo: 'Rimuovi Video',
                calculating: 'Calcolo in corso...',
                highQualityRender: 'Rendering Alta Qualità',
                processing: 'Elaborazione',
                frameByFrame: 'Fotogramma per fotogramma',
                browserNativeRender: 'Rendering Nativo del Browser',
                videoOutputTitle: 'Output Video di Alta Qualità',
                videoOutputDesc: 'Il nuovo video viene generato in formato MP4, con la stessa qualità del video originale',
                exportVideo: 'ESPORTA VIDEO',
                processingProgress: 'Elaborazione',
                generatingHQ: 'Generazione Video Alta Qualità',
                hqNote: 'Elaborazione fotogramma per fotogramma per la massima qualità. Potrebbe richiedere più tempo rispetto alla durata originale.',
                keepTabActive: 'Per favore mantieni questa scheda attiva. Cambiare scheda potrebbe mettere in pausa il rendering.',
                cancelGeneration: 'Annulla Generazione',
                cancelExport: 'Annulla Esportazione',
                cancelConfirm: 'Sei sicuro di voler annullare l\'esportazione? I progressi andranno persi.',
                yesCancel: 'Sì, Annulla',
                noContinue: 'No, Continua',
                abortGenerationTitle: 'Annullare la Generazione?',
                abortGenerationMsg: 'Sei sicuro di voler annullare la generazione del video? I progressi andranno persi.',
                yesAbort: 'Sì, Annulla',
                continue: 'Continua',
                removeVideoTitle: 'Rimuovere Video?',
                removeVideoMsg: 'Sei sicuro di voler rimuovere questo video? L\'azione non può essere annullata.',
                exportFailed: 'Esportazione fallita. Vedi la console per i dettagli.',
                originalFormat: 'Originale',
                videoFormat: 'Formato Video',
                rotation: 'Rotazione',
                audioSettings: 'Audio',
                keepAudio: 'Mantieni Audio',
                removeAudio: 'Rimuovi',
                replaceAudio: 'Sostituisci',
                selectAudio: 'Seleziona Audio',
                videoError: 'Questo formato o codec video non è supportato dal tuo browser. I vecchi file AVI (~2008 e precedenti) spesso utilizzano codec obsoleti. Prova a convertirlo in MP4 (H.264).',
            },
            photoverlay: {
                title: 'PhotOverlay',
                overlaySettings: 'Sovraimpressione',
                selectPhoto: 'Seleziona una foto per aggiungere overlay',
                uploadPhotos: 'Seleziona o trascina foto',
                awaitingSource: 'In attesa delle foto',
                removePhoto: 'Rimuovi Foto',
                location: 'Posizione',
                exportPhoto: 'ESPORTA FOTO',
                exporting: 'Esportazione...',
                removePhotoTitle: 'Rimuovere Foto?',
                removePhotoMsg: 'Sei sicuro di voler rimuovere questa foto? L\'azione non può essere annullata.',
                exportFailed: 'Esportazione fallita. Vedi la console.',
                panImage: 'Sposta Immagine',
                applyToAll: 'Applica questa sovraimpressione a tutte le foto',
                exportAllPhotos: 'ESPORTA TUTTE LE FOTO',
                applyToAllTitle: 'Applicare a Tutte le Foto?',
                applyToAllMsg: 'Questo sovrascriverà le impostazioni personalizzate delle altre foto. Sei sicuro?',
                yesApply: 'Sì, Applica',
                removeAllPhotosTitle: 'Rimuovere Tutte le Foto?',
                removeAllPhotosMsg: 'Sei sicuro di voler rimuovere tutte le foto? L\'azione non può essere annullata.',
                yesRemoveAll: 'Sì, Rimuovi Tutte',
                exportMode: 'Modalità Esportazione',
                individualFiles: 'File Singoli',
                singleArchive: 'Archivio Singolo (ZIP)',
            },
        },
    },
};

export type TranslationKeys = typeof translations.en;
